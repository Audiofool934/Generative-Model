...
for i = 1:size(E, 1),
    ...
    for j = 1:length(F),
        ...
        if (~isempty(indx)),
            ...
            % Get all possible assignments for the variables in the factor
            assignments = IndexToAssignment(1:length(F(j).val), F(j).card);

            % Find assignments that don't match the evidence
            % (indx is the position of variable v in F(j).var)
            inconsistent = assignments(:, indx) ~= x;

            % Set factor values for inconsistent assignments to 0
            F(j).val(inconsistent) = 0;
